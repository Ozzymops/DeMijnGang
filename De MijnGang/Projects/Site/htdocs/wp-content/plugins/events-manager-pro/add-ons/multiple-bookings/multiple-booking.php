...
class EM_Multiple_Booking extends EM_Booking{
    ...
    function add_booking( $EM_Booking ){
        if( empty($this->bookings[$EM_Booking->event_id]) ){
	        ...
	        // update booking hook
	        do_action('em_multiple_booking_updated', $EM_Booking->event_id, $EM_Booking, $this);
	        // recalculate price
    ...
    function remove_booking($event_id){
    	if( !empty($this->bookings[$event_id]) ){
		    ...
		    $EM_Booking = $this->bookings[$event_id];
		    unset($this->bookings[$event_id]);
		    // update booking hook
		    do_action('em_multiple_booking_updated', $event_id, $EM_Booking, $this);
	    ...
	    if( $result ){
			...
			if( !empty($booking_ids) && is_numeric($status) ){
			    ...
			    if( $result ){
				    ...
				    EM_Pro::log( array("Could not update status for MB Booking ID {$this->booking_id}", array('booking_ids' => $booking_ids, 'DB Error' => $wpdb->last_error, 'DB Query' => $wpdb->last_query)), 'multiple-bookings');
			    }
			}else{
	...
	function email( $email_admin = true, $force_resend = false, $email_attendee = true ){
		...
		add_filter('pre_option_dbem_bookings_notify_admin', 'EM_Multiple_Booking::admin_email', 100); // replace admin email with MB admin email
		$return = parent::email( $email_admin, $force_resend );
		do_action('em_multiple_booking_email_after_send', $this);
		remove_filter('pre_option_dbem_bookings_contact_email', '__return_zero');
		remove_filter('pre_option_dbem_bookings_notify_admin', 'EM_Multiple_Booking::admin_email');
		return $return;
	}
	
	public static function admin_email(){
		return get_option('dbem_bookings_notify_admin_mb');
	}
