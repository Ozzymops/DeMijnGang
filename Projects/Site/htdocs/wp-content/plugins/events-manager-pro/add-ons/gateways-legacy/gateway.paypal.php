...
class EM_Gateway_Paypal extends EM_Gateway {
	...
	function get_paypal_vars($EM_Booking){
		...
		if( get_option('em_'. $this->gateway . "_format_logo" ) !== false ){
			$paypal_vars['image_url'] = get_option('em_'. $this->gateway . "_format_logo" );
		}
		...
		if( $discount > 0 || $surcharges > 0 ){
			...
			foreach( $EM_Booking->get_tickets_bookings()->tickets_bookings as $EM_Ticket_Bookings ){ /* @var $EM_Ticket_Bookings EM_Ticket_Bookings */
	...
	function get_paypal_url(){
		$sandbox = get_option('em_'. $this->gateway . "_status" ) == 'test' || get_option('em_'. $this->gateway . "_status" ) == 'sandbox';
...
Events Manager
	...
	function is_sandbox(){
		return get_option('em_'. $this->gateway . "_status" ) === 'test';
	}
	...
	function mysettings() {
		...
		if( get_option('em_'. $this->gateway . '_api', false) === false ) {
		...
		<tbody>
		  <?php em_options_input_text( esc_html__('Success Message', 'em-pro'), 'em_'. $this->gateway . '_booking_feedback', sprintf(esc_html__('The message that is shown to a user when a booking is successful whilst being redirected to %s for payment.','em-pro'), 'PayPal') ); ?>
		  <?php em_options_input_text( esc_html__('Success Free Message', 'em-pro'), 'em_'. $this->gateway . '_booking_feedback_free', sprintf(esc_html__('If some cases if you allow a free ticket (e.g. pay at gate) as well as paid tickets, this message will be shown and the user will not be redirected to %s.','em-pro'), 'PayPal') ); ?>
		  <?php em_options_input_text( esc_html__('Thank You Message', 'em-pro'), 'em_'. $this->gateway . '_booking_feedback_completed', sprintf(esc_html__('If you choose to return users to the default Events Manager thank you page after a user has paid via %s, you can customize the thank you message here.','em-pro'), 'PayPal') ); ?>
		  <?php em_options_input_text( esc_html__('Payment Cancelled Message', 'em-pro'), 'em_'. $this->gateway . '_booking_feedback_cancelled', sprintf(esc_html__('This message will be displayed when users are redirected back from %s to a event booking or checkout page if they have clicked the cancel/back button on the website (not the back button on a browser), which would result in their temporary booking also being cancelled and deleted.','em-pro'), 'PayPal') ); ?>
		</tbody>
		</table>
		...
		<table class="form-table">
			<tbody>
				...
				<?php
					...
					$is_sandbox = get_option('em_'. $this->gateway . "_status") == 'test';
		...
		<tbody>		  
			...
			<tr valign="top">
				...
				<td>
					<input type="text" name="em_paypal_format_border" class="em-colorpicker em-paypal-border-color" value="<?php esc_attr_e(get_option('em_'. $this->gateway . "_format_border" )); ?>" style='width: 40em;' /><br />
			...
			<tr valign="top">
				...
				<td>
					...
					<br/><br/>
					<?php if( !empty($api_help_tip) ): ?><em><?php echo $api_help_tip; ?></em><?php endif; ?>
				</td>
			</tr>
			<tr valign="top">
	...
	function update() {
		...
		if( is_ssl() || (defined('EMP_PAYPAL_SSL_OVERRIDE') && EMP_PAYPAL_SSL_OVERRIDE) || (get_option('em_'. $this->gateway . "_status") == 'test' && !empty($_REQUEST['show_keys']) && wp_verify_nonce($_REQUEST['show_keys'], 'show_paypal_creds')) ){
			$gateway_options['em_paypal_rest_api'] = array('client_id', 'client_secret');
		}
