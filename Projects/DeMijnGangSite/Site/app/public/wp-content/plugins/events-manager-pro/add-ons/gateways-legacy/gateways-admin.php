...
class EM_Gateways_Admin{
	...
	public static function init(){
		...
		if( !empty($_REQUEST['page']) && ($_REQUEST['page'] === 'events-manager-options' || $_REQUEST['page'] === 'events-manager-gateways') && (!is_multisite() || is_super_admin()) ){
			add_action('network_admin_notices', 'EM_Gateways_Admin::legacy_notice');
			add_action('admin_notices', 'EM_Gateways_Admin::legacy_notice');
			if( !empty($_REQUEST['legacy-gateway-deactivate']) && wp_verify_nonce($_REQUEST['legacy-gateway-deactivate'], 'legacy-gateway-deactivate') ){
				EM_Options::site_set('legacy-gateways', false);
				wp_safe_redirect( admin_url('admin.php?page=events-manager-gateways') );
				exit();
			}
		}
	}
	...
	public static function legacy_notice(){
		...
		<div class="notice notice-warning">
			<p>You are currently using the Legacy Mode for payment gateways in Events Manager which reverts to our older Gateways API available in versions before <code>Events Manager Pro 3.2</code>. Newly released gateways will not work in legacy mode.</p>
			<?php if( defined('EMP_GATEWAY_LEGACY') && EMP_GATEWAY_LEGACY ): ?>
				<p><em>You currently have <code>define('EMP_GATEWAY_LEGACY', true);</code></em> defined in the wp-config.php file or elsewhere in your site code, remove this line to enable transition, or set it to <code>false</code> to force-disable legacy mode.</p>
			<?php else: ?>
				<p>If you are using our current gateways, you can begin transition seamlessly by clicking the button below, and gain access to all the extra new payment features. This can be reversed if things don't work as expected.</p>
				<p>If you are using third party gateway integrations that have not been updated to integrate with the new Gateways API in Events Manager Pro 3.2, please ensure they have been updated to support the latest gateway API before proceeding.</p>
				<?php if( is_multisite() ): ?>
				<p><em>This action will be applied to entire network of this MultiSite installation.</em></p>
				<?php endif; ?>
				<p><a class="button-primary" href="<?php echo esc_url( add_query_arg('legacy-gateway-deactivate', wp_create_nonce('legacy-gateway-deactivate')) ); ?>">Deactivate Legacy Mode</a></p>
			<?php endif; ?>
		</div>
		<?php
	}
	...
	public static function admin_options(){
		...
		?>
			...
			<div class="inside">
				<?php if( is_super_admin() ) echo '<p><em>Legacy Mode - Only site admins see this.</em></p>'; ?>
