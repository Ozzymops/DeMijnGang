...
class EM_Forms {
	...
	public static function admin_page(){
		...
		<div class='wrap'>
			...
			<div class="metabox-holder emp-forms-editor">
			...
			<div class="inside">
				<table class='form-table'>
					<?php 
						...
						em_options_radio_binary ( __( 'Show username profile fields?', 'em-pro' ), 'dbem_emp_booking_form_reg_show_username', __( 'Displays a non-editable field with the username of the currently logged in user. Usernames cannot be edited/changed in the booking form.', 'em-pro' ) );
...
class EM_Form extends EM_Object {
	...
	function get_formatted_value( $field, $field_value ){
		...
		switch( $field['type'] ){
			...
			case 'date':
			    ...
			    if( $field_value != 'n/a' && !empty($field_value) ){
		...
		return apply_filters('emp_form_get_formatted_value', $field_value, $field, $original_field_value, $this);
	}
	...
	public function is_normal_field( $field_or_id ){
	    ...
	    return is_array($this->form_fields) && array_key_exists($field_id, $this->form_fields) && !array_key_exists($field_id, $this->user_fields) && !in_array($field_id, ['user_email','user_name']) && !in_array($this->form_fields[$field_id]['type'], ['html', 'captcha']);
	}
	...
	function __toString(){
		...
		if( is_array($this->form_fields) ){
			foreach($this->form_fields as $field){
				$return .= $this->output_field($field);
			}
		}
			...
			case 'time':
				...
				<p class="input-group input-<?php echo $field['type']; ?> input-field-<?php echo $field['fieldid'] ?>">
					<label for='<?php echo $field['fieldid'] ?>'>
						<?php if( $field['type'] === 'checkbox' ) echo $this->output_field_input($field, $post); // checkboxes will show label ln right so things align better ?>
						<?php if( !empty($field['options_'.$tip_type.'_tip']) ): ?>
							<span class="form-tip" data-tippy-content="<?php echo esc_attr($field['options_'.$tip_type.'_tip']); ?>">
					...
					<?php if( $field['type'] !== 'checkbox' ) echo $this->output_field_input($field, $post); ?>
				</p>
				<?php
				break;
			case 'captcha':
							...
							<p class="input-<?php echo $field['type']; ?> input-user-field">
								<label for='<?php echo $field['fieldid'] ?>'>
									<?php if( !empty($field['options_reg_tip']) ): ?>
										<span class="form-tip" data-tippy-content="<?php echo esc_attr($field['options_reg_tip']); ?>">
											<?php echo $field['label'] ?> <?php echo $required  ?>
										</span>
									<?php else: ?>
										<?php echo $field['label'] ?> <?php echo $required  ?>
	...
	function output_field_input($field, $post=true){
		...
		switch($field['type']){
			...
			case 'tel' :
				?>
				<input type="tel" name="<?php echo $field_name ?>" id="<?php echo $field['fieldid'] ?>" class="input" value="<?php echo $default; ?>"  />
				<?php
				break;
			case 'textarea':
				$size = 'rows="2" cols="20"';
			...
			case 'date':
				...
				$date_type = !empty($field['options_date_range']) ? 'em-datepicker-range':'';
				$placeholder = !empty($field['options_date_range']) ? esc_attr__emp( 'Select date range') : esc_attr__emp( 'Select date');
				?>
				<span class="em-datepicker <?php echo $date_type; ?> em-event-dates">
					<input type="hidden" class="em-date-input em-date-start-end" aria-hidden="true" placeholder="<?php echo $placeholder ?>">
					<span class="em-datepicker-data">
						<input type="date" name="<?php echo str_replace('[%s]','[start]', $field_id); ?>" value="<?php echo !empty($default[0]) ? $default[0]:'' ?>" aria-label="<?php _e ( 'From ', 'events-manager'); ?>">
						<?php if( !empty($field['options_date_range']) ) : ?>
						<span class="separator"><?php echo $field['options_date_range_seperator']; ?></span>
						<input type="date" name="<?php echo str_replace('[%s]','[end]', $field_id); ?>" value="<?php echo !empty($default[1]) ? $default[1]:'' ?>" aria-label="<?php _e('to','events-manager'); ?>">
						<?php endif; ?>
					</span>
				</span>
				<?php
    			break;	
			case 'time':
			    $date_type = !empty($field['options_time_range']) ? 'em-time-range inline-inputs':'em-time-single';
	...
	function validate_field( $field_id, $value ){
		...
		if( is_array($field) ){
			...
			switch($field['type']){
				...
				case 'textarea':
				case 'tel':
					if( $field['type'] !== 'tel' || !class_exists('\EM\Phone') || !EM\Phone::is_enabled() ) {
						//regex
				...
				case 'tel':
					//non-empty match
					if( trim($value) == '' && !empty($field['required']) ){
						$this->add_error($err);
						$result = false;
					} else {
						// validate it if we're using intl phone input
						if ( class_exists('\EM\Phone') && !EM\Phone::validate( $value ) ) {
							$this_err = (!empty($field['options_tel_error'])) ? $field['options_tel_error']:$err;
							$this->add_error( $this_err );
						}
					}
					break;
				...
				case 'date':
				    ...
				    $start_date = !empty($dates[0]) ? $dates[0]:'';
				...
				default:
					...
					if( array_key_exists($field['type'], $this->user_fields) && self::validate_reg_fields($field) ){
						...
						if ( $field['type'] == 'user_email' ) {
							if( ! is_email( $value ) ){
								$this->add_error( emp__( '<strong>ERROR</strong>: The email address isn&#8217;t correct.') );
								$result = false;
	...
	function get_fields_map(){
		$map = array (
			...
			'options_tel_error','options_tel_tip',
	...
	public static function show_reg_fields( $field = false ){
		...
		if( !empty($field) ){
			...
			$hidden_fields = array('user_password'); //usernameas and passwords are always hidden once a user exists
			if( is_user_logged_in() && !EM_Bookings::$force_registration ){
				if( !get_option('dbem_emp_booking_form_reg_show_username') ){
					$hidden_fields[] = 'user_login';
				}
				if( !get_option('dbem_emp_booking_form_reg_show_email') ){
					$hidden_fields[] = 'user_email';
				}
				...
				if( $field_type == 'user_login' && (!is_user_logged_in() || EM_Bookings::$force_registration) && get_option('dbem_bookings_anonymous') ){
					$is_hidden_reg = true; //user login is always hiddeen for no-user mode bookings
				}
			}
		}
	...
	public static function validate_reg_fields( $field = false ){
		if( emp_is_manual_booking( true ) ) {	//short circuit if we're on a manual booking for a new user, we validate in that case
			// only login fields are exempt if in no-user mode, since we don't need it
			return !get_option('dbem_bookings_anonymous') || empty($field['fieldid']) || $field['fieldid'] !== 'user_login';
		}
				...
				<ul class="booking-custom-body">
					...
					<li class="booking-custom-item" <?php if( $field_key === 'blank_em_template' ){ echo 'id="booking-custom-item-template"'; }; ?>>
						...
						<div class='bc-col-type bc-col'>
							<select name="type[]" class="booking-form-custom-type">
								...
								<optgroup label="<?php _e('Customizable Fields','em-pro'); ?>">
									...
									<?php if ( defined('EM_PHONE_INTL_ENABLED') && constant('EM_PHONE_INTL_ENABLED') ) : ?>
									<option <?php self::input_default('type',$field_values,'select','tel'); ?> value="tel">phone</option>
									<?php endif; ?>
						...
						<div class='booking-custom-types'>
							...
							<!-- tel -->
							<?php if ( defined('EM_PHONE_INTL_ENABLED') && constant('EM_PHONE_INTL_ENABLED') ) : ?>
							<div class="bct-tel bct-options" style="display:none;">
								<div class="bct-field">
									<div class="bct-label"><?php _e('Tip Text','em-pro'); ?></div>
									<div class="bct-input">
										<input type="text" name="options_tel_tip[]" <?php self::input_default('options_tel_tip',$field_values); ?> />
										<?php if( EM_ML::$is_ml ) $this->editor_ml_fields_trigger('options_tel_tip'); ?>
										<em><?php _e('Will appear next to your field label as a question mark with a popup tip bubble.','em-pro'); ?></em>
									</div>
									<?php if( EM_ML::$is_ml ) $this->editor_ml_fields('options_tel_tip', $field_values); ?>
								</div>
								<div class="bct-field">
									<div class="bct-label"><?php _e('Error Message','em-pro'); ?></div>
									<div class="bct-input">
										<input type="text" name="options_tel_error[]" <?php self::input_default('options_tel_error',$field_values); ?> />
										<?php if( EM_ML::$is_ml ) $this->editor_ml_fields_trigger('options_tel_error'); ?>
										<em>
											<?php _e('If phone number does not validate, this error will be displayed.','em-pro'); ?>
											<br /><?php _e('Default:','em-pro'); echo ' <code>'.sprintf(get_option('dbem_emp_booking_form_error_required'), '[FIELD]').'</code>'; ?>
										</em>
									</div>
									<?php if( EM_ML::$is_ml ) $this->editor_ml_fields('options_tel_error', $field_values); ?>
								</div>
								<?php do_action('emp_forms_editor_tel_options', $this, $field_values); ?>
							</div>
							<?php endif; ?>
