    ...
    public static function settings(){
		...
		<script type="text/javascript">
			jQuery(document).ready(function($){
				$('input:radio[name="dbem_multiple_bookings"]').on('change', function(){
	...
	public static function emails(){
		...
		<div class="inside">
		    <div class="em-boxheader">
                <p><?php echo sprintf(__( 'When users make a booking in Multiple Bookings Mode these email templates will be used instead of the ones in the %s section. These emails summarize one or more bookings made via the checkout page.', 'em-pro' ), '<code>'.esc_html__emp( 'Booking Email Templates', 'events-manager').'</code>'); ?></p>
			    <p><?php echo sprintf(__( 'Any direct event booking (such as a manual booking by admins) or modification to an individual event booking will still trigger individual email templates from the %s section.', 'em-pro' ), '<code>'.esc_html__emp( 'Booking Email Templates', 'events-manager').'</code>'); ?></p>
			...
			<?php do_action('em_options_page_multiple_booking_email_templates_options_top'); ?>
			<table class='form-table'>
				<?php
				...
				<tbody class="em-subsection">
					<?php
					em_options_input_text ( __( 'Admin Email', 'em-pro'), 'dbem_bookings_notify_admin_mb', __( "All admin emails for a multiple booking will be sent here (leave blank to not send an email).", 'events-manager').' '.__('For multiple emails, separate by commas (e.g. email1@test.com,email2@test.com,etc.)','events-manager') );
					?>
				...
				<?php do_action('em_options_page_multiple_booking_email_templates_options_bottom'); ?>
				<?php echo $save_button; ?>
			</table>
		</div> <!-- . inside -->
		</div> <!-- .postbox -->
