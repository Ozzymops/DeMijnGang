...
class EM_Coupons extends EM_Object {
	...
	public static function init(){
		...
		if( get_option('dbem_multiple_bookings') ){ //multiple bookings mode
			...
			if( !empty($_REQUEST['action']) && $_REQUEST['action'] == 'em_coupon_apply' && !defined('DOING_AJAX') ){
				add_action('wp_loaded', array('EM_Coupons', 'cart_coupon_apply_fallback'), 100);
			}
		...
		add_action('em_bookings_table_cols_template', array('EM_Coupons', 'em_bookings_table_cols_template'), 10, 2);
		add_action('em_bookings_table_cols_template_groups', array('EM_Coupons', 'em_bookings_table_cols_template_groups'),10,1);
	...
	public static function em_cart_footer($EM_Multiple_Booking){
		...
		?>
		<form id='em-cart-coupons-form' class="em-cart-coupons-form em-cart-actions inline-inputs" name='cart-coupons-form' method='post' action='<?php echo apply_filters('em_checkout_coupons_form_action_url',''); ?>#em-booking'>
		 	...
		 	<div>
				...
				<button type="submit" class="em-coupon-code em-clickable">
					<span class="loading-content"><span class="em-icon em-icon-spinner"></span></span>
					<?php _e('Apply Discount','em-pro'); ?>
				</button>
			</div>
		</form>
		<?php
	...
	public static function em_bookings_table_cols_template_groups( $template_groups ) {
		$template_groups['payment']['fields'][] = 'coupon';
	...
	public static function build_sql_conditions( $args = array() ){
		...
		if( !empty($args['scope']) ){
			if ( is_array($args['scope']) ) {
				...
				$date_start = $args['scope'][0];
				$date_end = $args['scope'][1];
				if( !empty($date_start) && empty($date_end) ){
					// anything available from the start date onwards wheter or not already available
					$conditions['scope'] = "coupon_end >= CAST('$date_start' AS DATE) OR coupon_end IS NULL";
				}elseif( empty($date_start) && !empty($date_end) ){
					// anything available until the provided end date
					$conditions['scope'] = "( (coupon_start <= CAST('$date_end' AS DATE) OR coupon_start IS NULL ) AND (coupon_end >= CAST('$date_end' AS DATE) OR coupon_end IS NULL) )";
				}else{
					$conditions['scope'] = "( (coupon_start <= CAST('$date_end' AS DATE) OR coupon_start IS NULL ) AND (coupon_end >= CAST('$date_start' AS DATE) OR coupon_end IS NULL) )";
				}
			} elseif ( preg_match ( "/^[0-9]{4}-[0-9]{2}-[0-9]{1,2}$/", $args['scope'] ) ) {
				$conditions['scope'] = " ( coupon_start <= CAST('{$args['scope']}' AS DATE) OR coupon_start IS NULL ) AND ( coupon_end >= CAST('{$args['scope']}' AS DATE) OR coupon_end IS NULL )";
			}
		}
