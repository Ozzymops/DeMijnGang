...
class Email_Bookings {
	...
	public static function actions(){
		if( !empty($_POST['email_event_bookings']) && !empty($_POST['event_id']) && wp_verify_nonce($_POST['email_event_bookings'],'email_event_bookings_'.$_POST['event_id']) ){
			...
			if( !$EM_Event->can_manage('manage_bo'))
	...
	public static function email_page(){
		...
		$subject = !empty($_POST['subject']) ? wp_unslash($_POST['subject']) : get_option('dbem_email_bookings_default_subject');
		
		if ( (defined('EM_UNFILTERED_HTML') && constant('EM_UNFILTERED_HTML')) || current_user_can('unfiltered_html') ) {
			$message = !empty($_POST['message']) ? wp_unslash($_POST['message']) : get_option('dbem_email_bookings_default_body');
		} else {
			$message = !empty($_POST['message']) ? wp_kses_post( wp_unslash($_POST['message']) ) : get_option('dbem_email_bookings_default_body');
		}
		?>
		<div class='wrap'>
			<?php if( is_admin() ): ?>
			...
			<form action="" method="post" enctype="multipart/form-data" class="em-email-event-bookings">
				...
				<p>
					<textarea name="message" type="text" class="widefat" placeholder="<?php esc_attr_e('Email Message', 'events-manager-pro'); ?>" aria-label="<?php esc_attr_e('Email Message', 'events-manager-pro'); ?>" id="em_emails_form_message"><?php echo $message; ?></textarea>
				</p>
