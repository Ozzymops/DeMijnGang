...
class EM_Custom_Emails_Admin {
	...
	public static function emails_editor_js(){
		...
		<script type="text/javascript">
			jQuery(document).ready(function($){
				...
				$('.emp-cet select.emp-cet-status').on('change', function(){
					var el = $(this);
	...
	public static function get_default_emails(){
			...
			'default' => array(
				...
				'subgroups' => array(
					'admin'=>array(
						...
						'emails' => apply_filters('em_custom_emails_admin_default_emails_subgroup', array(
		...
		$emails = apply_filters('em_custom_emails_admin_default_emails', $emails);
		if( get_option('dbem_custom_emails_gateways') ){
	...
	public static function get_default_email_values( $EM_Gateway = false, $multiple_bookings = false ){
		...
		$email_values = array(
			$mb_key.'admin'=> apply_filters('em_custom_emails_admin_default_email_values', array(
			...
			), $multiple_bookings, 'admin'),
			$mb_key.'user'=> apply_filters('em_custom_emails_admin_default_email_values', array(
		...
		if( get_option('dbem_custom_emails_gateways') ){
    		...
    		foreach($gateways as $gateway => $gateway_name ){
			    ...
			    $gateway_email_values[$gateway.'-'.$mb_key.'admin'] = apply_filters('em_custom_emails_admin_default_email_values', $gateway_email_values[$gateway.'-'.$mb_key.'admin'], $multiple_bookings, 'admin', $gateway);
			    $gateway_email_values[$gateway.'-'.$mb_key.'user'] = apply_filters('em_custom_emails_admin_default_email_values', $gateway_email_values[$gateway.'-'.$mb_key.'user'], $multiple_bookings, 'user', $gateway);
