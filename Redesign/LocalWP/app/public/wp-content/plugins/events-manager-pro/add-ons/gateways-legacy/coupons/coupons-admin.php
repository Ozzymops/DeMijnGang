		...
		<div class="em-booking-options">
			...
			<script type="text/javascript">
			   jQuery(document).ready(function($){
				   $('#em-event-bookings-coupons-trigger').on('click', function(e){
					   e.preventDefault();
					   el = $(this);
	        ...
	        ?>
				...
				$coupon_args = array('owner'=>$owner, 'sitewide'=>0, 'eventwide'=>0 );
				...
				foreach( $coupons as $k => $EM_Coupon ){ /* @var EM_Coupon $EM_Coupon */
					if( !empty($EM_Coupon->coupon_end) && strtotime($EM_Coupon->coupon_end, current_time('timestamp')) <= current_time('timestamp') ){
						if( in_array($EM_Coupon->coupon_id, EM_Coupons::event_get_coupon_ids($EM_Event)) ){
							?>
							<input type="hidden" name="em_coupons[]" value="<?php echo esc_attr($EM_Coupon->coupon_id); ?>">
							<?php
						}
						unset($coupons[$k]);
					}
	            ...
	            if( count($coupons) > 0 ): foreach($coupons as $EM_Coupon): ?>
	                <label>
			...
			$coupon_args = array('owner'=>$owner, 'sitewide'=>1, 'eventwide'=>1, 'scope' => array($EM_DateTime->format('Y-m-d'), ''));
			$coupons = apply_filters('em_coupons_admin_meta_box_coupons', EM_Coupons::get($coupon_args), $coupon_args, $EM_Event );
	        if( count($coupons) > 0 ): ?>
